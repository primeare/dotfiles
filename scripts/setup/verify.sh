...
verify_installation() {
  ...
  if [[ "${INSTALL_LATEST_NODE:-true}" == "true" ]]; then
    fnm_current="$(fnm current)" || fail "Could not determine the active Node.js version"

    [[ -n "$fnm_current" && "$fnm_current" != "system" && "$fnm_current" != "none" ]] || \
      fail "Node.js is not managed by FNM"

    command -v node >/dev/null 2>&1 || fail "Node.js is missing"
    log "Node.js: $(node --version) (FNM: $fnm_current)"
  fi
}
